...
def main(_):
    ...
    print(f'we have total {len(df_filenames)} positive tiles')

    first_filename = df_filenames[0]
    second_filename = df_filenames[1]

    print("first_filename: ", first_filename)
    print("second_filename: ", second_filename)
    
    train_df = df[df.filename != first_filename]
    train_df = train_df[train_df.filename != second_filename]

    val_df = df.loc[df['filename'] == first_filename]
    test_df = df.loc[df['filename'] == second_filename]
